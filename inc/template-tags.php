...
function greatmag_posted_on() {
	...
	$meta = '<h5 class="post-meta"><a href="' . esc_url( get_the_permalink() ) . '" class="date">' . $time_string . '</a></h5>';
...
function greatmag_get_post_cats( $first_cat = false, $featured = false ) {
	if ( 'post' === get_post_type() ) {
		$cats = get_the_tags();  // [used to be: get_the_category(); template-parts/content.php]
